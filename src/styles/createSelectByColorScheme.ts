import type { UIKitColorScheme, UIKitTheme } from '@/types/ui';
...
const createSelectByColorScheme = (colorScheme: UIKitColorScheme): UIKitTheme['select'] => {
  return (options) => {
    const value =
      options[colorScheme ?? 'default'] ?? options.light ?? options.dark ?? options.default;
    if (!value) {
      throw Error('Not provided any selectable color scheme values');
    }
    return value;
  };
};
